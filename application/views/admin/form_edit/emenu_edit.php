  ...
  <div class="min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-4xl bg-white/40 backdrop-blur-2xl rounded-2xl shadow-lg p-8">
      ...
      <form action="<?= base_url('admin/update_action/' . $konten->id_konten) ?>" method="post"
        ...
        <!-- Deskripsi -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Komentar Menu</label>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Komentar Menu in English</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Foto Produk Menu</label>
            <input type="file" name="logo"
              class="block w-full border border-gray-800/50 focus:ring-black rounded-lg px-4 py-2 focus:ring-1">
            <?php if ($konten->logo): ?>
            <img src="<?= base_url('assets/img/' . $konten->logo) ?>" alt="Logo"
              class="mt-4 h-20 rounded-xl shadow-2xl object-cover">
            <?php endif; ?>
          </div>
        </div>
